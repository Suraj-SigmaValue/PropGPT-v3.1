import os

def apply_chat_interface():
    file_path = 'c_app.py'
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    # Locate the "Analysis Button" section
    start_ui_index = -1
    end_ui_index = -1
    
    for i, line in enumerate(lines):
        if 'st.write("**Analysis Query**")' in line:
            start_ui_index = i
        if 'if generate_btn:' in line:
            end_ui_index = i
            break
            
    if start_ui_index == -1 or end_ui_index == -1:
        print("Could not find UI markers. File might be different than expected.")
        return

    # Everything before the UI
    new_lines = lines[:start_ui_index]
    
    # Add is_query_relevant function
    relevance_func = [
        '\n',
        'def is_query_relevant(query: str, llm) -> bool:\n',
        '    """Check if user query is relevant to real estate analysis."""\n',
        '    relevance_prompt = f"""You are a relevance checker for a real estate analysis assistant.\n',
        '\n',
        'User Query: "{query}"\n',
        '\n',
        'Is this query related to real estate analysis, property data, market trends, pricing, sales, demand, supply, or similar real estate topics?\n',
        '\n',
        'Respond with ONLY "YES" or "NO".\n',
        '\n',
        'YES - if about real estate, properties, market analysis\n',
        'NO - if about unrelated topics like weather, sports, recipes, etc.\n',
        '\n',
        'Response:"""\n',
        '\n',
        '    try:\n',
        '        response = llm.invoke(relevance_prompt)\n',
        '        answer = response.content.strip().upper() if hasattr(response, \'content\') else str(response).strip().upper()\n',
        '        return "YES" in answer\n',
        '    except Exception as e:\n',
        '        logger.warning(f"Relevance check failed: {e}. Assuming relevant.")\n',
        '        return True\n',
        '\n'
    ]
    new_lines.extend(relevance_func)

    # Add Chat Interface Code
    chat_interface = [
        '# ---------------- CHAT INTERFACE ----------------\n',
        '\n',
        '# Initialize chat history\n',
        'if "messages" not in st.session_state:\n',
        '    st.session_state.messages = []\n',
        '\n',
        '# Display chat messages from history\n',
        'for message in st.session_state.messages:\n',
        '    with st.chat_message(message["role"]):\n',
        '        st.markdown(message["content"])\n',
        '\n',
        '# Accept user input\n',
        'if query := st.chat_input("Ask a question about the selected items..."): \n',
        '    st.session_state.messages.append({"role": "user", "content": query})\n',
        '    \n',
        '    with st.chat_message("user"):\n',
        '        st.markdown(query)\n',
        '\n',
        '    with st.chat_message("assistant"):\n',
        '        response_text = None  # Initialize to avoid NameError\n',
        '        # Relevance check first\n',
        '        try:\n',
        '            temp_llm = get_llm(selected_response_llm_provider)\n',
        '            if not is_query_relevant(query, temp_llm):\n',
        '                irrelevant_msg = """I appreciate your question, but I\'m specifically designed to help with **real estate analysis** topics such as:\n',
        '\n',
        '- Property market trends and pricing\n',
        '- Sales and demand analysis  \n',
        '- Location and project comparisons\n',
        '- Supply and inventory metrics\n',
        '\n',
        'Could you please ask a question related to real estate or property analysis? 🏡"""\n',
        '                \n',
        '                st.warning(irrelevant_msg)\n',
        '                st.session_state.messages.append({"role": "assistant", "content": irrelevant_msg})\n',
        '                st.stop()\n',
        '        except Exception as e:\n',
        '            logger.warning(f"Relevance check error: {e}")\n',
        '\n'
    ]
    new_lines.extend(chat_interface)
    
    logic_lines = lines[end_ui_index + 1:]
    
    cleaned_logic_lines = []
    skip = False
    for line in logic_lines:
        if 'if not query or not query.strip():' in line:
            skip = True
            cleaned_logic_lines.append('        # Query guaranteed from chat_input\n')
            continue
        if skip and 'st.info(f"Using default query: {query}")' in line:
            skip = False
            continue
        if skip:
            continue
        cleaned_logic_lines.append(line)
        
    # Indent the logic lines
    indented_logic = []
    for line in cleaned_logic_lines:
        if line.strip():
            indented_logic.append('        ' + line)
        else:
            indented_logic.append(line)
            
    new_lines.extend(indented_logic)
    
    final_lines = []
    history_appended = False
    
    for i, line in enumerate(new_lines):
        if 'footer_html =' in line and not history_appended:
            # Insert history append before footer with check
            final_lines.append('        if response_text:\n')
            final_lines.append('            st.session_state.messages.append({"role": "assistant", "content": response_text})\n\n')
            history_appended = True
            final_lines.append(line)
        else:
            final_lines.append(line)
            
    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(final_lines)
    
    print("Successfully applied chat interface transformation with fix.")

if __name__ == "__main__":
    apply_chat_interface()
